...
//	need app icon
//	need app name
//	only create one sand spider from one bombbox
...
//		Copy over text format processing code from Gemicus
//	difficulty or configuration object that has constants, i.e spawn rate/number of plants generated/spider eating speed...
//		can up dificulty by decreasing seconds in myPath for boxes
//	(EDIT SPRITE)Spider needs rightward facing sprites for default,moving and dead
...
//		(IMPORTANT)Have lives and damage system setup from spiders, just change lives to numberOfLives and change sprite accordingly like rocks
//	Sound Effects
